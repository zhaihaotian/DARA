...
def configuration(args):
    ...
    length_max_words = getattr(args, 'length_max_words', 0)
    if length_max_words < 0 or (length_max_words and args.rewards != 'three'):
        raise ValueError('--length-max-words must be nonnegative and requires --rewards three.')
    ...
    environment['LENGTH_MAX_WORDS'] = str(length_max_words)
...
def main():
    ...
    parser.add_argument('--length-max-words', type=int, default=0,
                        help='Inclusive upper bound for binary think-word reward; 0 uses the dense length reward')
    ...
    record = dict(method=args.method, seed=args.seed, group_size=args.group_size,
                  ...
                  length_max_words=args.length_max_words,
