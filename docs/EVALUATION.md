...
Length≤16实验使用 `evaluation/run.py --version v4 --temperature 0.6 --top-p 1 --length-max-words 16` 与 `--temperature 1 --top-p 1 --length-max-words 16` 两档。配置保存到inference.json，评分自动读取阈值；`groups.length`为二值Length Reward，`groups.length_le_16`为达标比例，输出→case→category→group的平均顺序与下述协议一致。运行和汇总命令见 [HANDOFF_LENGTH16.md](HANDOFF_LENGTH16.md)。
