...
| 项目 | 结果 |
|---|---|
| CPU测试 | 35项通过：独立NumPy算法参考、PPO query权重与梯度、动态累积、100step/checkpoint调度、三奖励接线、通道密度日志、两卡配对启动、DVAO seed3过程训练入口、评测权重与格式/长度统计 |
| 过程模型保存 | `--save-freq 10` 仅修改保存间隔；训练循环在steps10/20/…/100保存，验证保持steps0/10/…/100 |
| 本轮排队与评测接线 | 调度28项测试、V4过程评测5项测试通过，覆盖任务优先级、到期后新attempt、A100/H100分配、同节点端口、156个目标模型、逐step统计与最终表复用 |
| 两卡配对训练 | 单节点四卡与两个Ray节点各两卡的训练配置仅节点布局不同；Slurm参数、卡型与allocation检查通过，GPU通信实跑待获配资源 |
| H100两卡训练 | 3B DVAO seed0已完成100steps并保存全部过程模型；固定batch回放确认当前两rank梯度权重与四rank不同，后续两卡训练需完成四路逻辑分组接线与验收 |
