...
| H100两卡训练 | 四路逻辑分组已接通；四rank与两rank分布式CPU回放的microbatch顺序一致，四次更新的梯度最大差2.98e-8、参数最大差1.16e-10。GPU/FSDP对照由下一份H100分配自动执行，通过后训练，详见TWO_GPU_LOGICAL_FOUR.md。此前3B DVAO seed0的100step训练与结果保留 |
