...
from verl.trainer.ppo import multi_reward_algos
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1,
                      algorithm_config=None, global_step=1):
    ...
    if adv_estimator == 'gae':
    ...
    elif adv_estimator in ('gdpo', 'dara'):
        index = data.non_tensor_batch['uid']
        ...
        if adv_estimator == 'gdpo':
            new_advantage = channel_advantages[0]
            for channel in channel_advantages[1:]:
                new_advantage = new_advantage + channel
        else:
    ...
    elif adv_estimator in multi_reward_algos.METHODS:
        response_length = data.batch['responses'].size(-1)
        response_mask = data.batch['attention_mask'][:, -response_length:]
        method_config = (algorithm_config or {}).get(adv_estimator, {})
        advantages, method_metrics, extra = multi_reward_algos.compute_multi_reward_advantage(
            method=adv_estimator,
            correctness=data.batch['token_level_scores_correctness'],
            format_reward=data.batch['token_level_scores_format'],
        ...
        data.meta_info['multi_reward_metrics'] = method_metrics
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            test_gen_batch_padded, pad_size = pad_dataproto_to_divisor(
                test_gen_batch, self.config.trainer.get('logical_world_size', self.actor_rollout_wg.world_size))
            ...
            if 'jsonl' in self.config.trainer.logger:
                prompts = self.tokenizer.batch_decode(test_batch.batch['prompts'], skip_special_tokens=True)
                responses = self.tokenizer.batch_decode(test_batch.batch['responses'], skip_special_tokens=True)
                for i, response in enumerate(responses):
                    validation_records.append({
                        'step': self.global_steps,
                        'index': int(test_batch.non_tensor_batch['index'][i]),
                        'prompt': prompts[i], 'response': response,
                        'reward': reward_tensor[i].sum().item(),
                        'format': format_tensor[i].sum().item(),
                        'correctness': correctness_tensor[i].sum().item(),
                        'length': length_tensor[i].sum().item(),
                    })
        ...
        if validation_records:
            validation_dir = os.path.join(self.config.trainer.default_local_dir, 'eval')
            os.makedirs(validation_dir, exist_ok=True)
            with open(os.path.join(validation_dir, f'step_{self.global_steps:03d}.jsonl'), 'w') as output:
    ...
    def init_workers(self):
        ...
        if self.config.algorithm.adv_estimator == 'gae':
            ...
            self.use_critic = False
        elif self.config.algorithm.adv_estimator == 'dara':
            self.use_critic = False
        elif self.config.algorithm.adv_estimator in multi_reward_algos.METHODS:
            self.use_critic = False
        else:
            raise NotImplementedError
    ...
    def _balance_batch(self, batch: DataProto, metrics, logging_prefix='global_seqlen'):
        ...
        world_size = self.config.trainer.get('logical_world_size', self.actor_rollout_wg.world_size)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if self.global_steps >= self.total_training_steps:
                    if self.val_reward_fn is not None and not (
