...
def compute_channel_densities(channel_advantages, index):
    ...
    pis = []
    active_counts = []
    for A_j in channel_advantages:
        # sample_energy: (N,) total |advantage| per rollout
        sample_energy = A_j.abs().sum(dim=-1)
        # group_energy per group: sum over the G rollouts of that group
        active = 0
        for gid in group_ids:
            rows = id2rows[gid]
            group_energy = sample_energy[rows].sum()
            if group_energy > 1e-8:
                active += 1
        pi_j = active / num_groups if num_groups > 0 else 0.0
        pis.append(pi_j)
...
def compute_dara_combined_advantage(channel_advantages,
    ...
    for pi_j in pis:
        if pi_j <= 0.0:
            # dead channel: no signal anywhere. Weight is irrelevant since A_j is
            # all-zero on every active row; use w_max as the (capped) limit.
            w_j = w_max
        else:
            w_j = min(w_max, math.sqrt(pi_ref / pi_j))
        weights.append(w_j)
    ...
    metrics = {}
    for name, pi_j, w_j, ac in zip(channel_names, pis, weights, active_counts):
        metrics[f"dara/pi_{name}"] = float(pi_j)
        metrics[f"dara/w_{name}"] = float(w_j)
        metrics[f"dara/active_groups_{name}"] = float(ac)
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange,
    ...
    Args:
        ...
        query_weight: optional `(bs,)` tensor
            GD2PO-Hard query retention weights, applied after clipping while
            retaining the original eos_mask denominator.
    ...
    if query_weight is not None:
        clipped_losses = clipped_losses * query_weight.unsqueeze(-1)
