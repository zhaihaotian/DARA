...
def _select_rm_score_fn(data_source):
    if data_source == 'openai/gsm8k':
        ...
        from verl.utils.reward_score import math_deepscaler
        return math_deepscaler.compute_score
    else:
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, graded_format=False) -> None:
        self.tokenizer = tokenizer
        self.num_examine = num_examine  # the number of batches of decoded responses to print to the console
        # Densify the format channel for training only. The binary format
        # reward fires on 0.6% of rollouts, so it is constant inside almost
        # every prompt group and no multi-channel aggregator can see it. The
        # validation manager keeps graded_format False, so the reported
        # endpoint stays the original all-or-nothing reward and remains
        # comparable to earlier runs. See MULTICHANNEL_AGGREGATION_NOTE.md.
        self.graded_format = graded_format
    ...
    def __call__(self, data: DataProto, step: int):
        ...
        for i in range(len(data)):
            ...
            if "deepscaler" in data_source:
                score, fomrat_score, correctness_score, length_score = compute_score_fn(
                    solution_str=sequences_str, ground_truth=ground_truth, step=step,
            ...
            if self.graded_format:
                graded = rlla.format_graded_score(sequences_str, ground_truth)
                score = score - fomrat_score + graded
                fomrat_score = graded
...
def main_task(config):
    # --- reproducibility: set a global seed (default 0), overridable via SEED env var ---
    import os as _os, random as _random
    import numpy as _np
    _seed = int(_os.environ.get('SEED', 0))
    _random.seed(_seed)
    _np.random.seed(_seed)
    torch.manual_seed(_seed)
    torch.cuda.manual_seed_all(_seed)
    print(f'!!! Global seed set to {_seed} (dataloader shuffle + torch RNG)')
    ...
    reward_fn = RewardManager(tokenizer=tokenizer, num_examine=0,
                              graded_format=os.getenv('FORMAT_GRADED', '0') == '1')
