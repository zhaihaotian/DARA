...
8×A100 80GB SXM4租机任务按docs/RENTAL_A100_80G_SXM4_AGENT.md执行，每个实验使用四张物理GPU、四个训练rank，两个实验共享GPU0–3，另两个共享GPU4–7，目标为四个实验并行。执行agent落实独立Ray/端口、每实验显存预算以及并发通信验证，再启用共享布局。第一调度规则是任何时候最大化利用所有已租GPU：任务结束立即补位，训练资源不足或剩余租期较短时转入checkpoint评测，使用tmux持续维护任务。其他项目现有调度按各自配置执行。
